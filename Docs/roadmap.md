...
| Gap | Impact | Note |
| --- | :---: | --- |
| ~~PS3.15 profile options~~ | **B** | ~~`DICOMDeidentificationProfile` is a conservative preset, not the Basic Application Level Confidentiality Profile with its options (Retain Longitudinal Temporal Information, Retain UIDs, Clean Pixel Data, …).~~ **Done** — see Phase 5, item 10. `DICOMConfidentialityProfile` resolves all 631 rows of Table E.1-1 (generated by `Tools/generate_deidentification_profile.py`) plus its ten Retain/Clean options. DICOMKit still does not clean free text/structured content/graphics or modify pixel data — a selected Clean option is applied as a conservative removal instead. |
| ~~Burned-in annotation handling~~ | **B** | ~~`(0028,0301)` is not inspected and pixel-level text is never detected, so "anonymized" exports can still carry identifiers in the image.~~ **Done** — see Phase 5, item 10. `DICOMConfidentialityProfile.deidentify(_:replacement:)` throws `DICOMError.burnedInAnnotationPresent` when `(0028,0301)` is `YES`, and surfaces a warning (rather than silently proceeding) when it is altogether absent. DICOMKit still cannot detect or redact pixel-level text itself. |
...
10. **PS3.15 profile options** — **Done** (4 commits: `f903608`, `ab6d5ed`, `6b39d73`, `1f48dd7`)
    - Generate PS3.15 Table E.1-1 (631 attribute rows: 627 exact tags, 3 repeating-group ("xx") tags matched by mask, 1 non-tag row dropped and recorded as such) from the official DocBook XML via the new `Tools/generate_deidentification_profile.py`, into `DICOMDeidentification.generated.swift`. — `f903608` "Generate the PS3.15 confidentiality table"
    - Add `DICOMDeidentificationAction`, `DICOMDeidentificationOption`, and `DICOMConfidentialityProfile`, which resolves the Basic Profile action for a tag and applies any selected Retain/Clean option's override (later Table E.1-1 column wins when two selected options both apply to the same attribute); `makeAnonymizer(replacement:)` builds a `DICOMAnonymizer` from a profile, mapping `Clean` to outright removal since DICOMKit does not implement true cleaning. Reimplemented `DICOMDeidentificationProfile.basicApplicationLevelConfidentiality` on top of the same generated table instead of its previous ~20-tag hand list, preserving its existing dummy-replacement behavior for Patient's Name/Patient ID. — `ab6d5ed` "Model the confidentiality profile and its options"
    - Inspect Burned In Annotation `(0028,0301)` and refuse — via the new `DICOMError.burnedInAnnotationPresent` — to claim de-identification for an instance that declares burned-in text. Add `DICOMConfidentialityProfile.deidentify(_:replacement:)`, returning a `DeidentificationResult` whose `warnings` says so when `(0028,0301)` is altogether absent rather than explicitly `NO`, so that weaker claim can't be missed by only looking at the returned dataset. — `6b39d73` "Refuse to claim de-identification for burned-in text"
    - Record what was applied — Patient Identity Removed `(0012,0062)`, De-identification Method `(0012,0063)`, and De-identification Method Code Sequence `(0012,0064)` populated from PS3.16 CID 7050 — since a profile claim is only meaningful if it is recorded (PS3.15 Section E.1.1). — `1f48dd7` "Record the de-identification method"
...
12. **Performance benchmarks** — **Done** (2 commits: `53d705d`, and this section's own "Record the performance baseline") — measures large-series open time and per-frame decode, so later work has a baseline.
    - Add `DICOMKitBenchmark`, an executable target depending on `DICOMKit` alone, with a dependency-free `ContinuousClock` harness (warmup pass discarded, fixed repetition count, median/min/max rather than a mean — a timing assertion belongs nowhere near `swift test`, which this deliberately stays out of). Benchmarks: dataset parse (`CT_small.dcm`, a synthesized large flat dataset, and deeply nested sequences), series open (64 instances), metadata-only open, per-frame decode, and render. Verified `swift build`, `swift test` (485 tests unchanged), and the iOS device-triple build all still succeed with the new target present. — `53d705d` "Add a performance benchmark harness"
    - Record a baseline in [benchmarks.md](benchmarks.md) (machine, OS, Swift version, release build, iteration count, median/min/max per benchmark) and document how to re-run it in README's Development section. Metadata-only open's saving turned out to scale with how much of the file is Pixel Data — barely visible on `CT_small.dcm`'s tiny 32 KB image but roughly 16x on a synthesized 32 MB multi-frame dataset — and rendering one frame costs roughly 85x decoding one, which is the more useful finding for anyone chasing scroll-performance complaints than the raw numbers alone. — "Record the performance baseline" (a commit can't record its own final hash; see `git log` for it)
